...
void spider_rocket(edict_t* self, monster_muzzleflash_id_t mz)
{
	...
	if (blindfire)
	{
		...
		{
			monster_fire_rocket(self, start, dir, 25, rocketSpeed, mz);
		}
		else
		{
			vec = target;
			...
			if (!(trace.startsolid || trace.allsolid || (trace.fraction < 0.5f)))
			{
				monster_fire_rocket(self, start, dir, 50, rocketSpeed, mz);
			}
			else
			{
				vec = target;
...
void spider_rocket_heat(edict_t* self, monster_muzzleflash_id_t mz)
{
	vec3_t	forward, right;
	vec3_t	start;
	vec3_t	dir;
	...
	if (blindfire)
	{
		if (!(trace.startsolid || trace.allsolid || (trace.fraction < 0.5f)))
		{
			monster_fire_heat(self, start, dir, 50, rocketSpeed, mz, 0.075f);
		}
		else
		{
			vec = target;
	...
	else
	{
		if (trace.fraction > 0.5f || trace.ent->solid != SOLID_BSP)
		{
			monster_fire_heat(self, start, dir, 50, rocketSpeed, mz, 0.15f);
		}
	}
}
...
{
	spider_rocket(self, MZ2_ARACHNID_RAIL1);
}
...
{
	spider_rocket_heat(self, MZ2_ARACHNID_RAIL1);
}

void spider_rocket_right2(edict_t* self)
{
	spider_rocket_heat(self, MZ2_ARACHNID_RAIL2);
}
