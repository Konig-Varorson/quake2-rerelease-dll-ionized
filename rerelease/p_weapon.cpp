...
void weapon_deatomizer_fire(edict_t* self)
{
	vec3_t	 end;
	edict_t* enemy;
	trace_t	 tr;
...
void Weapon_Deatomizer(edict_t* ent)
{
	...
	Weapon_Generic(ent, 4, 9, 29, 34, pause_frames, fire_frames, weapon_deatomizer_fire);
}
...
void weapon_flamegun_fire(edict_t* ent)
{
...
constexpr int32_t DISCHARGER_DM_DMG = 15;
constexpr int32_t DISCHARGER_SP_DMG = 15;

void weapon_discharger_fire(edict_t* ent)
{
	bool firing = (ent->client->buttons & BUTTON_ATTACK);
	bool has_ammo = ent->client->pers.inventory[ent->client->pers.weapon->ammo] >= ent->client->pers.weapon->quantity;
...
void Weapon_Discharger(edict_t* ent)
{
	...
	Weapon_Repeating(ent, 8, 12, 42, 47, pause_frames, weapon_discharger_fire);
}
...
void weapon_perforator_fire(edict_t* ent)
{
	...
	for (i = 0; i < shots; i++)
	{
		...
		fire_flechette(ent, start, dir, damage, 1350, kick);
	}
	G_UnLagCompensate();
...
void Weapon_Perforator(edict_t* ent)
{
	...
	Weapon_Repeating(ent, 4, 31, 61, 64, pause_frames, weapon_perforator_fire);
}
...
void Weapon_Disintegrator(edict_t* ent)
{
	...
	Weapon_Generic(ent, 8, 18, 43, 51, pause_frames, fire_frames, weapon_disint_fire);
}